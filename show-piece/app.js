...
const updateCopy = function (copy) {
    ...
    if (copy.trim().length > 0){
        appendImage(copy, '.copy', 'afterbegin')
    }
...
const appendImage = function (word, selector, position) {
    let imageEl = document.querySelector('.images')
    let copyEl = document.querySelector(selector)
    ...
    copyEl.insertAdjacentElement(position, li);
}
...
window.addEventListener('keyup', function (e) {
    ...
    if (e.key === 'Backspace') {
        ...
        appendImage(copy, '.copy', 'beforebegin')
        copy = ''
